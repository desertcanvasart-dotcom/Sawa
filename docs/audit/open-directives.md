...
> **19.2 DONE 10 Aug 2026** — `docs/audit/entity-disclosure-sweep.md`.
> **19.1 and 19.3 remain blocked on the registered name.**
>
> | surface | result |
> |---|---|
> | rendered pages | **23 of 23** public routes carry it |
> | transactional email | **12 of 12** real templates carry it |
> | production database | **0 rows**, across 121 text columns |
> | source | **five independent authors**, four different sentences |
>
> The earlier note said "at least 10 `site/*.html` files". The files are 22; the
> **authors** are five — the other 21 footers are generated by `applyFooter`.
>
> **The response-derived method earned its keep:** `/itineraries`, `/blog` and
> `/booking` disclose the entity in **JSON-LD with no footer at all**. A rename
> that fixed visible copy and missed `server/brand.js` would leave the old name
> in the structured data Google reads, on three pages, with nothing on screen to
> contradict it. `server/email.js` is the other likely miss — it hardcodes its
> own sentence rather than reading `BRAND`.
>
> **Not swept:** `sitemap.xml` was unreachable, so `publicRoutes()` fell back to
> its 23 hard-coded routes and tour/package/blog detail pages were not reached.
> Stated rather than assumed away.
>
> `server/entity-disclosure.test.js` ratchets the five authors — a sixth fails
> the build, a stale one fails too, and the declared list **is** 19.1's
> checklist.
