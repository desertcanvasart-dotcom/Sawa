-- Sawa — settlements (044), on its own.
--
-- For a database where payments (043) is already in place. Paste into the
-- Supabase SQL editor and run once. Safe to run again, and one transaction:
-- if any statement fails, nothing is applied.
--
-- The last statement records 044 as applied — and 043 too, if its table is
-- there — so the app's own migration check sees both.
...
INSERT INTO schema_migrations (name) VALUES ('044_settlements') ON CONFLICT (name) DO NOTHING;
INSERT INTO schema_migrations (name)
  SELECT '043_booking_payments' WHERE to_regclass('public.booking_payments') IS NOT NULL
