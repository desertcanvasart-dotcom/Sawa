...
| `check:applied-schema` | **production's `schema_migrations`**, over a read-only session | **VERDICT** — the only check that queries the deployed database | Four states, because connecting successfully to the WRONG database produces a confident wrong answer: applied / not-applied / could-not-reach / wrong-target. The target is established before the query runs — an answer from an unidentified database is not a weaker answer, it is a different question. Weaker without `PRODUCTION_DB_HOST`: it can then only rule out local and private addresses, and says so. |
